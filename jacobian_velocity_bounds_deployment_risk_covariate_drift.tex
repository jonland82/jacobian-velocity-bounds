...
Finally, Table~\ref{tab:hazard-ablation} checks whether the matched hazard score carries more information than drift magnitude or gain alone. This simple correlation test is intentionally modest: drift magnitude has the largest average next-block correlation, while the matched score remains positively correlated with next-block risk and is more model-aware. We therefore treat $h_t$ as a theorem-motivated monitoring proxy, not as an empirically dominant detector.

\begin{table}[t]
\caption{Hazard-score ablation on selected DTR real-deployment trajectories. Correlations are averaged over Air Quality and Tetouan.}
\centering
\setlength{\tabcolsep}{5pt}
\small
\begin{tabular}{lcc}
\toprule
Score & Next-block risk & Same-block risk \\
\midrule
Drift only ($s_t^2$) & $0.18$ & $-0.17$ \\
Gain only ($g_t$) & $0.02$ & $0.05$ \\
Matched ($h_t=s_t^2g_t$) & $0.11$ & $0.06$ \\
\bottomrule
\end{tabular}
\label{tab:hazard-ablation}
\end{table}
