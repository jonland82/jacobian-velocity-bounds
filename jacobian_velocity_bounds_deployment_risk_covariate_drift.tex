...
\author{
\IEEEauthorblockN{Jonathan R. Landers\,\orcidlink{0000-0003-1872-6179}}
\IEEEauthorblockA{
\href{mailto:jonathan.robert.landers@gmail.com}{jonathan.robert.landers@gmail.com}\\
\href{https://orcid.org/0000-0003-1872-6179}{ORCID: 0000-0003-1872-6179}
}
}
...
\textit{Assumption A3 (directional domination by the score Jacobian).} There exists $\beta>0$ such that for almost every $x$ and every $v\in\R^d$,
\[
|\nabla g_\theta(x)^\top v| \le \beta \|J_f(x)v\|.
\]
...
\begin{remark}[When A3 holds and when it fails]
\label{rem:a3}
A3 holds in the natural composition case. If $g_\theta(x)=h(f_\theta(x))$, then $\nabla g_\theta(x)=h'(f_\theta(x))\nabla f_\theta(x)$ and A3 follows with $\beta=\sup|h'|$. For Bernoulli cross-entropy against a fixed soft target, $|h'|\le 1$, giving $\beta=1$.
...
Lemma~\ref{lem:poincare} says volatility is impossible without derivative energy. The next result identifies the geometric driver of that energy.

\begin{theorem}[Jacobian-velocity control of risk volatility]
\label{thm:jv}
Assume A1--A3 and suppose $f_\theta$ is a ReLU network, so $J_f(x)$ exists almost everywhere. Then $r$ is absolutely continuous and
\[
\Var_U(r(U)) \le \frac{\beta^2 T}{\pi^2}\int_0^T \E\|J_f(X_t)\dot X_t\|^2dt.
\]
\end{theorem}
...
\section{Experiments}
\subsection{Synthetic theorem sanity check}
...
Figure~\ref{fig:theory} plays the theorem-linked role. Each point is one trained model, plotted by empirical risk volatility versus the derivative-energy quantity from Lemma~\ref{lem:poincare}. The points sit below the diagonal, as they should, and DTR moves the cloud toward the lower-left corner. Across all eight models, mean volatility falls from $3.13\times10^{-3}$ to $2.62\times10^{-4}$, while mean directional gain falls from $44.9$ to $1.91$.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/figure_2_synthetic_theorem.png}
  \caption{Synthetic theorem sanity check. Each point is one trained model evaluated over the drifting deployment path. The horizontal axis is the derivative-energy quantity from Lemma~\ref{lem:poincare}, and the vertical axis is empirical volatility $\Var_U(r(U))$. DTR shifts models toward lower energy and lower volatility.}
  \label{fig:theory}
\end{figure}
