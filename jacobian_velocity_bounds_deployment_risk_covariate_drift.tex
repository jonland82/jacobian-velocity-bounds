...
\title{\huge Jacobian-Velocity Bounds for Deployment Risk\\Under Covariate Drift}
\author{
...
Finally, the work draws on two technical lines. The regularization side is related to Jacobian-based control of robustness \cite{sokolic-jacobian,kim-jacobian} and to empirical evidence linking sensitivity to generalization \cite{novak-sensitivity}. Here the principal object is tangent energy in likely drift directions; anisotropic DTR retains a smaller orthogonal penalty as protection against residual motion. The monitoring side is related to shift detection, uncertainty under shift, and sequential change detection \cite{rabanser-shift,ovadia-shift,wu-rscusum,cao-xie-allerton2015,xie-lowrank-allerton2016,krishnamurthy-allerton2022,cooper-allerton2024}. Our hazard score is a model-aware proxy matched to the same drift geometry used in training rather than a sequentially optimal detector.
...
The crucial qualification is the distinction between model-mediated and conditional-risk variation. Covariate shift alone does not imply exact A3: even when $P_0(Y\mid X)$ is fixed over time, moving through $x$ can change that conditional in ways not transmitted through the score Jacobian. Proposition~\ref{prop:residual} isolates this effect in $Q$, and the stress test shows that it is a real boundary. DTR targets the Jacobian-mediated terms in \eqref{eq:residual-lowrank-bound}; a large remainder instead calls for a richer model, labels, or adaptation. Simultaneous concept shift remains outside the setup. The result is not specific to ReLU networks: ReLU models are admissible when deployment almost surely avoids kink sets for almost every time, while smooth models fit directly.

The studies play distinct roles. Two synthetic experiments test the temporal bound and its low-rank implication; the remainder stress test probes the boundary of Jacobian-only control. The gas-sensor study supplies the main field validation: its predeployment basis occupies four of $128$ directions, outperforms matched random subspaces, and shows that a small orthogonal guardrail can improve both risk and volatility over pure DTR. Air Quality and Tetouan extend the evidence to regression, emphasizing nuisance-aligned construction and prospective transfer.

The low-rank analysis depends on estimating a drift subspace. The gas study uses ten ambient-random controls, while the Air Quality and Tetouan studies use one fixed control; broader evidence remains valuable. The gas basis also uses labeled calibration exposures and known concentrations, which are natural in controlled calibration but not universal. Concentration composition changes across deployment batches despite class-balanced evaluation. Rolling $V_t$ remains appropriate for monitoring, whereas training-time DTR requires a historical or predeployment basis unless the model is retrained. The Jacobian-velocity bound is useful because it is actionable, not because it is always tight.
...
The author used software tools, including AI assistance, for figure preparation, algebraic checks, and editing. The author is responsible for all content and conclusions. Disclosure of interest. The author reports no conflict of interest. Funding. No funding was received for this research.
